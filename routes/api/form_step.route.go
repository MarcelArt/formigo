...
func SetupFormStepRoutes(api fiber.Router, auth *middlewares.AuthMiddleware) {
	h := api_handlers.NewFormStepHandler(repositories.NewFormStepRepo(database.GetDB()))

	g := api.Group("/:org_id/form-step")
